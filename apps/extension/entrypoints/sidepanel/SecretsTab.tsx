...
export function SecretsTab() {
  ...
  useEffect(() => {
    storageService.getItem<Record<string, string>>('local:local:secrets')
      .then((res) => {
        if (res) {
          setSecrets(res);
        } else {
          ...
          storageService.setItem('local:local:secrets', initialSecrets)
  ...
  const addSecret = () => {
    ...
    setSecrets(updated);
    storageService.setItem('local:local:secrets', updated)
